...
Env: `GOOGLE_API_KEY`, `GOOGLE_VIDEO_MODEL` (omni), `GOOGLE_VEO_MODEL`, `OPENAI_IMAGE_MODEL`, `LAB_VIDEO_PROVIDER=omni|veo|fake` (+ `LAB_PLANNER/LAB_IMAGE_PROVIDER=fake` for offline runs). API: `POST/GET /lab/videos`, `GET/DELETE /lab/videos/{id}`, `POST …/generate-images | keyframes/{i}/regenerate | animate`, `GET …/keyframes/{i}/image | segments/{i}/video | video`.
