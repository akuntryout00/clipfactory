...
def test_example_persona_loads_with_identity_tools_products():
    p = load_persona("indie_maker")
    assert p.identity.name == "Alex" and p.identity.age == 34 and "US" in p.identity.location
...
def test_persona_block_carries_identity_tools_policy_and_closing():
    ...
    assert "Alex" in txt and "34" in txt and "engineer" in txt and "first person" in txt.lower()
