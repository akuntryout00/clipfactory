...
def pick_look(rng: random.Random, scene_index: int) -> SceneLook:
    ...
    return SceneLook(
        zoom=round(zoom, 3), zoom_dir=zoom_dir, oversample=round(rng.uniform(1.0, 1.06), 3), jitter_x=rng.random(), jitter_y=rng.random()
    )
...
def scene_vf(look: SceneLook, duration: float, w: int = OUT_W, h: int = OUT_H, fps: int = OUT_FPS) -> str:
    ...
    if look.zoom_dir == 0 or look.zoom <= 0:
        ...
        zoom_chain = f",scale=w='trunc({w}*{factor}/2)*2':h='trunc({h}*{factor}/2)*2':eval=frame,crop={w}:{h}"
