...
def _body(tid="myshort_v1"):
    return {
        ...
        "duration": {"min": 15, "target": 17, "max": 22},
        "sections": [{"type": "hook", "weight": 0.3, "guidance": "punch"}, {"type": "body", "weight": 0.7, "guidance": "explain"}],
        "voiceover": True,
        "caption_style": "dynamic_center",
        "music_category": "minimal",
        "closing": "End on a question.",
        "shot_duration": {"min": 1.5, "max": 4.0},
        "overlays": {"min": 1, "max": 2},
    }
...
def test_create_rejects_bad_weights_and_duplicate_and_bad_id(client):
    ...
    bad_id = _body("../evil")
    assert c.post("/templates", json=bad_id).status_code == 422
...
def test_update_and_delete_template(client):
    ...
    upd = _body()
    upd["name"] = "My Short 2"
    upd["sections"][0]["weight"] = 0.4
    upd["sections"][1]["weight"] = 0.6
    ...
    mism = _body("other_v1")
    assert c.put("/templates/myshort_v1", json=mism).status_code == 422
