...
from app.schemas.pipeline import ScriptOutput, ScriptSection, VideoJSON, WordTiming
...
def test_assign_assets_builds_valid_video_json(session, mini_assets, tmp_path: Path):
    ...
    persona = load_persona("young_professional")
    text = (
        "Most people still take meeting notes by typing on a laptop at a desk. "
    ...
    vj = assign_assets(
        ...
        scenes=scenes,
        words=words,
        voice_audio="voice_v1.mp3",
        voice_duration=words[-1].end,
        caption_style=load_caption_style("dynamic_center"),
        seed=7,
    )
