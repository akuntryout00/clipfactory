...
1. **Describe** the video + length (3–25 s; ≤10 s = a single clip with 2 keyframes) + optional style, pick the video model (price estimate shown on the button, `GET /lab/estimate`) → an LLM writes a storyboard: a *style guide* for character/wardrobe/palette consistency and N keyframes (N = segments + 1; segments are 4–8 s each).
...
3. **Animate (Google Gemini Omni Flash via the Interactions API, default `gemini-omni-flash-preview`, up to ~10 s per clip)** — Omni does not interpolate between two frames, so each segment uses keyframe N as the literal `<FIRST_FRAME>` and keyframe N+1 as an `<IMAGE_REF_0>` reference for how the shot should end, with a single-continuous-shot prompt; clips (720p, with ambient audio) are normalised to 1080×1920/30 fps and concatenated into `final.mp4`. Alternative provider `LAB_VIDEO_PROVIDER=veo` (Veo 3.1, true first/last-frame interpolation, 8 s clips). “Animate again” re-animates all segments (`?force=true`). Per-segment **Edit & redo**: re-animate one clip with a new motion prompt, or apply an Omni **conversational edit** to the existing clip (`previous_interaction_id`, e.g. “make the sky golden, keep everything else the same”); the final video is rebuilt automatically. **Change length** (`PUT …/duration`): same segment count → keyframes kept and clips re-animated; different count → storyboard re-planned and keyframes regenerated.
**Video models** are chosen per video (New-video modal / “Compare with…” clone): Gemini Omni (`omni`), Veo 3.1 (`veo`), and via **fal.ai** (`FAL_KEY`): `fal:minimax-h3`, `fal:seedance-2.0`, `fal:seedance-2.0-fast`, `fal:seedance-2.0-1080p`, `fal:seedance-2.5`, `fal:kling-3.0-std`, `fal:kling-3.0-pro` (registry in `app/lab/providers.py::FAL_MODELS`; all use first+last keyframe, 9:16). `GET /lab/providers` lists availability. Env: `GOOGLE_API_KEY`, `FAL_KEY`, `GOOGLE_VIDEO_MODEL` (omni), `GOOGLE_VEO_MODEL`, `LAB_FAL_MODEL`, `OPENAI_IMAGE_MODEL`, `LAB_VIDEO_PROVIDER` (default for new videos; `fake` + `LAB_PLANNER/LAB_IMAGE_PROVIDER=fake` for offline runs). API: `POST/GET /lab/videos`, `GET/DELETE /lab/videos/{id}`, `POST …/generate-images | keyframes/{i}/regenerate | animate`, `GET …/keyframes/{i}/image | segments/{i}/video | video`.
