...
def test_upload_single_video_creates_asset_file_and_row(client, mini_assets, tmp_path):
    ...
    a = r.json()
    assert a["file"] == "indie_maker/desk/my_clip_1.mp4" and a["approved"] is True and a["duration"] > 2.5
    assert "notebook" in a["tags"] and a["description"] == "hand closes notebook"
    assert (mini_assets / "indie_maker" / "desk" / "my_clip_1.mp4").is_file()
    ...
    assert r2.status_code == 201 and r2.json()["file"] == "indie_maker/desk/my_clip_1_2.mp4" and r2.json()["approved"] is False
