...
| **B-roll** | grid with thumbnails, usage counts, category filter, search; **Add video** (upload one clip into a category with description/tags/approval and a **usable-range trimmer**: drag start/end handles on the preview, “Set start/end here”, “Preview range”); click a clip → same trimmer + edit tags/action/location/shot/mood/quality/approved, or delete it; Import folder, Enrich with AI |
